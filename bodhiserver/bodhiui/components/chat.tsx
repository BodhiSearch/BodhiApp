  ...
  const { messages, input, setInput, isLoading, append, reload } = useChat({
    ...
    onFinish: async () => {
      if (!!messages && messages.length == 0) {
        // sleep for 1s as it takes a bit to update the database
        if (newChatId != null)
          await router.push(RouteChat(newChatId));
