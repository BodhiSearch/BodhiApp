...
# Use llama.cpp Vulkan base image as runtime foundation
FROM ghcr.io/bodhisearch/llama.cpp:latest-vulkan AS runtime
...
ARG CI_DEFAULT_VARIANT=vulkan
ARG CI_BUILD_VARIANTS=vulkan,cpu
...
# Create defaults.yaml with Vulkan-optimized configuration
COPY <<EOF /app/defaults.yaml
# BodhiApp Default Configuration for Vulkan
...
# Build configuration - Vulkan variant
BODHI_EXEC_LOOKUP_PATH: /app/bin
BODHI_EXEC_TARGET: x86_64-unknown-linux-gnu
BODHI_EXEC_VARIANTS: vulkan,cpu
BODHI_EXEC_VARIANT: vulkan
BODHI_EXEC_NAME: llama-server

# Server arguments
BODHI_LLAMACPP_ARGS: "--jinja --no-webui --keep 24"
BODHI_LLAMACPP_ARGS_VULKAN: "--n-gpu-layers 999 --vulkan"
...
# Health check (base image includes Vulkan availability check)
