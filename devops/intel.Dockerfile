...
# Use llama.cpp Intel base image as runtime foundation
FROM ghcr.io/bodhisearch/llama.cpp:latest-intel AS runtime
...
ARG CI_DEFAULT_VARIANT=intel
ARG CI_BUILD_VARIANTS=intel,cpu
...
# Set up Intel GPU environment variables
...
# Create defaults.yaml with Intel GPU-optimized configuration
COPY <<EOF /app/defaults.yaml
# BodhiApp Default Configuration for Intel GPU (SYCL)
...
# Build configuration - Intel variant
BODHI_EXEC_LOOKUP_PATH: /app/bin
BODHI_EXEC_TARGET: x86_64-unknown-linux-gnu
BODHI_EXEC_VARIANTS: intel,cpu
BODHI_EXEC_VARIANT: intel
BODHI_EXEC_NAME: llama-server

# Server arguments
BODHI_LLAMACPP_ARGS: "--jinja --no-webui"
BODHI_LLAMACPP_ARGS_INTEL: "--n-gpu-layers -1 --threads 8 --batch-size 2048 --ubatch-size 512 --no-mmap --mlock"
...
# Health check (base image includes Intel GPU availability check)
