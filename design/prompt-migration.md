...
- **Form pages** → `Create API Model.html` + `create-api-model-app.jsx`, and `Create New Local Model v4.html` + `local-model-app.jsx` + `local-model.css`. Study how the JSX uses `.bf-shell` → `.bf-topbar` (breadcrumb only) → `.bf-scroll` → `.bf-container` → `.bf-card` → sections → `.bf-footer`, and how page-specific styling (quant table, model picker) lives in a small page CSS file using a page prefix while everything generic comes from `bodhi-form.css`.
- **List / 3-column pages** → `Bodhi Models.html` (full-width main, flat rows, right detail panel, both columns resizable).
- **2-column vanilla page** → `Bodhi App Settings.html`.
...
- `Create Fallback Model.html` (+ `create-fallback-model-app.jsx`, `create-fallback-model.css`) — **do this first**; it still rides the old `create-api-model.css`, and its topbar is ~8px off the gridline. Note `create-api-model.css` is *shared* with the API page, so migrate Fallback off it cleanly (mirror how Local Model got its own `local-model.css`).
- `New App Token.html`
- `Bodhi Access Request.html`
...
### Remaining (6)
- Forms: Create Fallback Model, New App Token, Bodhi Access Request
