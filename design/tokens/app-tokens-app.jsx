/* ═══════════════════════════════════════════════════
   ...
   In sync with the App Access Request domain:
     ...
     • Each token has a read-only "View full request" link to a read-only
       App-Access-Request page, plus the at-a-glance panel summary.
═══════════════════════════════════════════════════ */
      ...
      <div className="tk-act">
        {exchanged ? (
          <button className="tk-replacement-link" title="Go to replacement token"
            ...
            Replacement <Ic name="arrow-right" size={12} />
          </button>
        ) : (
    ...
    <div className="dp-panel">
      ...
      {exchanged && (
        <div className="dp-exchange dp-exchange-old">
          ...
          <div>
            <div className="dp-exchange-t">Invalidated by token exchange</div>
            <div className="dp-exchange-s">This token was submitted for an upgrade and is no longer valid. It was replaced by a new token.</div>
            <button className="dp-exchange-link" onClick={() => onSelectId(token.exchangedFor)}>View replacement token <Ic name="arrow-right" size={12} /></button>
          </div>
        </div>
      )}
      {token.supersedes && (
        <div className="dp-exchange dp-exchange-new">
          ...
          <div>
            <div className="dp-exchange-t">Issued via token exchange</div>
            <div className="dp-exchange-s">Replaced an earlier token after an approved upgrade request.</div>
            <button className="dp-exchange-link" onClick={() => onSelectId(token.supersedes)}>View previous token <Ic name="arrow-right" size={12} /></button>
          </div>
        </div>
      )}
      ...
      <div className="dp-foot">
        <a className="dp-btn dp-btn-outline" href={viewUrl} target="_blank" rel="noopener"><Ic name="external-link" size={14} /> View full request</a>
        ...
        {exchanged ? (
          ...
          <>
            {confirmDelete ? (
              <button className="dp-btn dp-btn-danger" style={{ borderColor: 'hsl(var(--destructive))', background: 'rgba(220,38,38,.05)', color: 'hsl(var(--destructive))' }}
                      onClick={() => onDelete(token.id)}><Ic name="trash-2" size={14} /> Confirm remove</button>
            ) : (
              <button className="dp-btn dp-btn-danger" onClick={() => setConfirmDelete(true)}><Ic name="trash-2" size={14} /> Remove from list</button>
            )}
          ...
          <>
            ...
            {confirmDelete ? (
              <button className="dp-btn dp-btn-danger" style={{ borderColor: 'hsl(var(--destructive))', background: 'rgba(220,38,38,.05)', color: 'hsl(var(--destructive))' }}
                      onClick={() => onDelete(token.id)}><Ic name="trash-2" size={14} /> Confirm revoke</button>
            ) : (
              <button className="dp-btn dp-btn-danger" onClick={() => setConfirmDelete(true)}><Ic name="trash-2" size={14} /> Revoke &amp; delete</button>
            )}
            {confirmDelete && <div className="dp-field-hint" style={{ textAlign: 'center' }}>Revoking is permanent. Click again to confirm.</div>}
          </>
        )}
      </div>
    </div>
