...
<!doctype html><html lang="en" data-theme="light"><head>
  ...
  <div id="root"></div>
  <!-- pinned React + Babel (copy exact tags+integrity from Bodhi Models.html) -->
...
- `Bodhi MCP Discover v2.html` (subPage `discover`)
- `Bodhi MCP Playground.html` (subPage `my-instances` or a playground sub-page — check `SHELL_NAV`)
- `Bodhi MCP New Instance.html` (subPage `new-mcp`) — this is a **form**, use `bodhi-form.css`.
...
- `App Tokens.html` + `app-tokens-app.jsx` — list page (`section="api-keys"`, subPage `app-tokens`).
- `Access Requests.html` + `access-requests-app.jsx` — list page (subPage `access-requests`).
- `Manage Users.html` + `manage-users-app.jsx` — list page (`section="settings"`, subPage `manage-users`).
- `New App Token.html` + `new-app-token-app.jsx` + `model-access-picker.jsx` — form (subPage `new-token`).
- `Bodhi Access Request.html` + `access-request-app.jsx` + `model-access-picker.jsx` — form/review page.
- `Bodhi App Settings.html` (vanilla) — settings page (`section="settings"`, subPage `app-settings`); React-ify like Chat.
