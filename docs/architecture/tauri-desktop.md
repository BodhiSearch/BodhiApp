...
- [`system-overview.md`](system-overview.md) - System architecture and crate organization
- [`crates/services/CLAUDE.md`](../../crates/services/CLAUDE.md) - Backend service patterns and integration

**FOR FRONTEND INTEGRATION:**
- [`crates/bodhi/src/CLAUDE.md`](../../crates/bodhi/src/CLAUDE.md) - React component patterns for desktop UI
...
In production desktop (and other `native`-feature builds), the Vite frontend is built to `crates/bodhi/out/` and baked into the binary via `include_dir!`. The embed lives in `crates/lib_bodhiserver/src/ui_assets.rs`:

```rust
pub static EMBEDDED_UI_ASSETS: Dir<'static> =
    include_dir!("$CARGO_MANIFEST_DIR/../bodhi/out");
```

Because the bundle is compiled in, embedded modes serve a stale UI until the frontend is rebuilt (`make build.ui-rebuild`). During development, the dev server proxies `/ui/*` to Vite instead (HMR, no rebuild). See the root `CLAUDE.md` "UI Development Workflow" for the three dev loops.
...
│        Vite + React + TypeScript + TanStack            │
├─────────────────────────────────────────────────────────┤
│               Tauri Core (Rust)                        │
...
- **Frontend / React component patterns** - [`crates/bodhi/src/CLAUDE.md`](../../crates/bodhi/src/CLAUDE.md)
- **Desktop + container dual-mode crate** - [`crates/bodhi/src-tauri/CLAUDE.md`](../../crates/bodhi/src-tauri/CLAUDE.md)

---

*For detailed Tauri configuration and advanced patterns, see the official Tauri documentation. For backend integration, see [`crates/services/CLAUDE.md`](../../crates/services/CLAUDE.md).*
