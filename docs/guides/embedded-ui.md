...
The BodhiApp UI is a **Vite + React application** (TanStack Router for client-side routing, TanStack Query v5 for server state) that runs embedded within the desktop app and is accessible via web browser at `http://localhost:1135`. Key characteristics:

- **Client-Side SPA**: Complete single-page application with file-based client-side routing
