...
**Status:** Deferred prompt/idea — kickoff for a future planning session, not a committed plan.
...
11. **Docs** — crate-level CLAUDE.md updates for `services`, `routes_app`, `bodhi`; `../architecture/security.md` note on single-node refresh assumption.
