...
| Auth Type | Route Access | Check 1 | Check 2 | Check 3 | Check 4 |
|-----------|--------------|---------|---------|---------|---------|
| Session | ✅ All endpoints | app_toolset_configs (admin enabled) | - | - | user_toolset_configs (API key) |
| API Token | ❌ 401 at route | - | - | - | - |
| External OAuth | ✅ List + Execute | app_toolset_configs (admin enabled) | app_client_toolset_configs (registered) | scope_toolset-* in token | user_toolset_configs (API key) |

**Note:** API tokens (`bodhiapp_*`) are blocked at the route level and never reach the authorization middleware.
...
CREATE TABLE IF NOT EXISTS app_client_toolset_configs (
    ...
    toolsets_json TEXT NOT NULL,  -- JSON array: [{"id":"...","scope":"...","scope_id":"...","added_to_resource_client":true}]
...
pub struct AppAccessRequest {
  ...
  pub toolset_scope_ids: Option<Vec<String>>,  // Keycloak scope UUIDs to enable on resource-client
}
```
...
pub struct AppClientToolset {
  pub id: String,      // Toolset identifier (e.g., "builtin-exa-web-search")
  pub scope: String,   // Scope name (e.g., "scope_toolset-builtin-exa-web-search")
  pub scope_id: String,              // Keycloak client scope UUID
  pub added_to_resource_client: Option<bool>,  // True if scope added to resource-client
}
```

**Handler Logic:**
1. If `version` provided, check DB cache by `app_client_id`
2. If cache hit and `config_version` matches `version`:
   - Check if all requested `toolset_scope_ids` have `added_to_resource_client=true`
   - If yes, return cached data
   - If no, proceed to Keycloak call
3. Otherwise, call Keycloak `/resources/request-access` with `toolset_scope_ids`
4. Store/update response in `app_client_toolset_configs` table
5. Return response to caller

**See:** [09.1-keycloak-toolset-scope-transfer.md](./09.1-keycloak-toolset-scope-transfer.md) for the Keycloak extension that handles `toolset_scope_ids`.
...
Full authorization logic for toolset execution:
...
- User ID header is set from `sub` claim

### /apps/request-access
- Cache hit returns cached data without Keycloak call
- Cache miss calls Keycloak and stores result
- Version mismatch with `toolset_scope_ids` not yet added triggers Keycloak call
- `toolset_scope_ids` forwarded to Keycloak for scope registration

### toolset_auth_middleware
- Session auth: checks app-level + user config only
- OAuth auth: checks all 4 levels (app + app-client + scope + user)
- Missing toolset scope returns 403
- Unregistered app-client returns 403
- Toolset not configured returns 400

### Route-Level Auth (E2E)
- API tokens return 401 for all toolset endpoints
- OAuth tokens can list toolsets (filtered by scope)
- OAuth tokens can execute tools (with scope check)
- OAuth tokens cannot access config endpoints (401)

**E2E Test File:** `crates/lib_bodhiserver_napi/tests-js/specs/toolsets/toolsets-auth-restrictions.spec.mjs`
