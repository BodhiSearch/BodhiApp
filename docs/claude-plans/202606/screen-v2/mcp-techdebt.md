...
| Rail tabs **About / Capabilities / Connection / Connect / Metadata** | Only About-ish data exists | Single-scroll rail (matches API-Models rail), not tabs. Sections: Description (`details`), Connection (`endpoint_url`/`transport`/`external_link`). **Provenance dropped:** `source`/`sources`/`first_seen_at`/`last_scraped_at` are scrape-internal and are being **deleted from the backend** (don't leak) → no Metadata section; the UI reads none of those fields. `verified`/`featured` surface in the header. Hide null sections. |
