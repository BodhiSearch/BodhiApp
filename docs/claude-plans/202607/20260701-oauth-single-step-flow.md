...
- **`crates/services/src/db/sea_migrations/m20250101_000009_app_access_requests.rs`** — ~~Edit in place: remove the `flow_type` and `redirect_uri` column definitions (no new migration; no prod data).~~ **SUPERSEDED — do not copy this step.** The in-place edit (commit `979b1cbf`) broke deployed DBs; `000009` was reverted and the drop moved to new migration `000026`. Committed migrations are immutable — see `crates/services/src/db/CLAUDE.md`.
