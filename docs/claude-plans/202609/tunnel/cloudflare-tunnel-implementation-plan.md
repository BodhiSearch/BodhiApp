...
**Date:** 2026-09-16 · **Trunk:** `main` @ `d4268ddb` (every `file:line` below re-read at that commit) · **Current research:** [`docs/research/tunnel/README.md`](../../../research/tunnel/README.md). This historical plan cites an earlier research layout; use the current snapshot for provider and implementation claims.
...
[`named-tunnel-operating-model.md`](../../../research/tunnel/named-tunnel-operating-model.md) records the current credential and DNS-routing decision. Traced in `cloudflared` 2026.9.1 source: `credentials.User::Client()` builds one `cfapi.RESTClient` from the cert's four fields, and every call — `POST /accounts/{id}/cfd_tunnel`, `GET`/`DELETE` the same, and **`PUT /zones/{zone}/tunnels/{id}/routes`** — uses that single bearer. `login → create → route dns → run` needs no second credential on a single-owner account. The tunnel-permissions page's "additional DNS/Load Balancer permissions" language is about the **Cloudflare account role** of a restricted team member, not a second credential.
...
[`bodhiapp-integration-and-risks.md`](../../../research/tunnel/bodhiapp-integration-and-risks.md) records the current request-origin contract. `X-Forwarded-Proto: https` reaches the origin, and `Host` arrives as the public hostname unless `originRequest.httpHostHeader` overrides it.
...
**Locked decisions honored:** named tunnels only, own zone · gate default `is_native()` · instance-wide, any admin · multi-origin model, **never** `BODHI_PUBLIC_HOST = tunnel host` · CLI control plane · connector readiness via `/ready` · `TUNNEL_TOKEN` environment credential · `cert.pem` remains administrator-managed. Current research: [named tunnel operating model](../../../research/tunnel/named-tunnel-operating-model.md) and [BodhiApp integration](../../../research/tunnel/bodhiapp-integration-and-risks.md).
...
| **2 (was 3)** | Scoped API token via two prefilled deep links | Historical alternative. The current operating model uses `cloudflared login`; see [named tunnel operating model](../../../research/tunnel/named-tunnel-operating-model.md). |
| **3 (was 2)** | Self-managed Cloudflare OAuth (PKCE **public** client) | Historical alternative. It is not part of the current supported scope. |
...
Two levels, deliberately separate. See the current status and origin contract in [BodhiApp integration and risks](../../../research/tunnel/bodhiapp-integration-and-risks.md).
...
One resolver replaces the ad-hoc composition at `routes_auth.rs:88-103` and the inline loop at `routes_setup.rs:132-168`. The current callback contract is in [BodhiApp integration and risks](../../../research/tunnel/bodhiapp-integration-and-risks.md).
...
- **Shutdown.** Stop the connector before the HTTP listener exits. The current lifecycle contract is in [named tunnel operating model](../../../research/tunnel/named-tunnel-operating-model.md).
...
Use the connector invocation in [named tunnel operating model](../../../research/tunnel/named-tunnel-operating-model.md).
...
- **cloudflared process layer.** Follow the current detection, credential, and supervision rules in [named tunnel operating model](../../../research/tunnel/named-tunnel-operating-model.md).
...
- **UI.** Follow the current UX and exposure contract in [BodhiApp integration and risks](../../../research/tunnel/bodhiapp-integration-and-risks.md).
