...
/// Tests streaming tool calling with Qwen3 model.
/// Verifies that the model correctly streams tool calls in OpenAI-compatible SSE format.
#[rstest::rstest]
#[awt]
#[tokio::test]
#[timeout(Duration::from_secs(5 * 60))]
#[serial_test::serial(live)]
async fn test_live_tool_calling_streamed(
  ...
  // Send streaming request expecting a tool call
  let response = client
    .post(&chat_endpoint)
