...
export function AppNavigation() {
  ...
  return (
    <nav className={cn('flex items-center p-2', isMobile ? 'w-auto' : 'w-64')} data-testid="app-navigation">
      <DropdownMenu>
        <DropdownMenuTrigger asChild>
          <Button
            variant="ghost"
            className={cn('flex items-center gap-2 h-auto py-2', isMobile ? 'w-10 px-0' : 'w-full')}
          ...
          >
            {isMobile ? (
              ...
              <>
                {currentItem.item.icon && <currentItem.item.icon className="size-4" />}
                <div className="flex-1 text-left space-y-0.5">
                  <p className="font-medium">{currentItem.parent?.title || currentItem.item.title}</p>
                  {currentItem.parent && <p className="text-sm text-muted-foreground">{currentItem.item.title}</p>}
                </div>
