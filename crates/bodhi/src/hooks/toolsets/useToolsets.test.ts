...
describe('useListToolsets', () => {
  ...
  it('caches toolsets data', async () => {
    ...
    const { result, rerender } = renderHook(() => useListToolsets(), {
      wrapper: createWrapper(),
    });
...
describe('useGetToolset', () => {
  ...
  it('handles error response', async () => {
    ...
    const { result } = renderHook(() => useGetToolset('invalid-id'), {
      wrapper: createWrapper(),
    });
...
describe('useListToolsetTypes', () => {
  it('fetches toolset types successfully', async () => {
    ...
    expect(result.current.data?.types[0].toolset_type).toBe('builtin-exa-search');
