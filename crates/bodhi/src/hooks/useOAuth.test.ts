...
describe('useOAuthInitiate', () => {
  it('handles successful OAuth initiation', async () => {
    ...
    expect(mockOnSuccess).toHaveBeenCalledWith(
      expect.objectContaining({
        data: { auth_url: 'https://oauth.example.com/auth?client_id=test' },
        status: 401,
  ...
  it('handles 303 redirect with Location header', async () => {
    const mockOnSuccess = vi.fn();
    const mockOnError = vi.fn();
    ...
    expect(mockOnSuccess).toHaveBeenCalledWith(
      expect.objectContaining({
        status: 303,
        headers: expect.objectContaining({ location: 'https://example.com/redirected' }),
      })
    );
    expect(mockOnError).not.toHaveBeenCalled();
  });
