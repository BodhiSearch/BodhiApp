...
/// `for_alias` returns a `MockAiApiClient` that delegates `forward_request_with_method`
/// to `response_fn`, so tests focus on response flow without re-mocking factory plumbing.
