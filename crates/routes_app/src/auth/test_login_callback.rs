...
async fn test_auth_callback_handler(temp_bodhi_home: TempDir) -> anyhow::Result<()> {
  // Create a token with the correct scope that matches what auth_initiate uses
  let claims = json! {{
    ...
    "scope": "email openid profile roles", // Sorted scope that matches auth_initiate
...
async fn test_auth_callback_handler_state_not_in_session(
  ...
  assert_eq!(
    "auth_route_error-session_info_not_found",
...
async fn test_auth_callback_handler_state_mismatch(temp_bodhi_home: TempDir) -> anyhow::Result<()> {
  ...
  assert_eq!(
    "auth_route_error-state_digest_mismatch",
