use crate::mcps::{
  mcp_oauth_discover_as, mcp_oauth_discover_mcp, mcp_oauth_dynamic_register,
  mcp_oauth_tokens_destroy, mcp_oauth_tokens_show, DynamicRegisterRequest, DynamicRegisterResponse,
...
async fn test_router_for_oauth_discovery(
  ...
  Ok(
    Router::new()
      .route("/mcps/oauth/discover-as", post(mcp_oauth_discover_as))
      .route("/mcps/oauth/discover-mcp", post(mcp_oauth_discover_mcp))
      .route(
...
async fn test_router_for_oauth_tokens(mock_mcp_service: MockMcpService) -> anyhow::Result<Router> {
  ...
  Ok(
    Router::new()
      .route("/mcps/oauth-tokens/{token_id}", get(mcp_oauth_tokens_show))
      .route(
