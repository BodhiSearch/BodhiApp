...
// Auth tier tests (merged from tests/routes_api_token_auth_test.rs)
...
#[tokio::test]
async fn test_token_endpoints_reject_insufficient_role(#[case] role: &str) -> anyhow::Result<()> {
...
#[case::manager("resource_manager")]
#[case::admin("resource_admin")]
#[tokio::test]
async fn test_token_list_endpoint_allows_eligible_roles(#[case] role: &str) -> anyhow::Result<()> {
  use crate::test_utils::{build_test_router, create_authenticated_session, session_request};
  let (router, app_service, _temp) = build_test_router().await?;
  let cookie = create_authenticated_session(app_service.session_service().as_ref(), &[role]).await?;
  let response = router
