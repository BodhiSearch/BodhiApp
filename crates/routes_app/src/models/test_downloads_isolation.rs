...
async fn isolation_router(
  ...
  let router = Router::new()
    .route("/bodhi/v1/modelfiles/pull", get(models_pull_index))
    .route("/bodhi/v1/modelfiles/pull/{id}", get(models_pull_show))
...
async fn test_cross_tenant_download_show_isolation(
  ...
  let response = router
    ...
    .oneshot(
      Request::builder()
        .uri(format!("/bodhi/v1/modelfiles/pull/{}", download_a_id))
        .body(Body::empty())?
        .with_auth_context(auth_a.clone()),
    )
    .await?;
