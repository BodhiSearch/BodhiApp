...
impl From<ApiKeyUpdateAction> for services::ApiKeyUpdate {
  fn from(action: ApiKeyUpdateAction) -> Self {
    match action {
      ApiKeyUpdateAction::Keep => services::ApiKeyUpdate::Keep,
      ApiKeyUpdateAction::Set(key) => {
        services::ApiKeyUpdate::Set(key.as_option().map(|s| s.to_string()))
      }
    }
  }
}
