...
**OAuth consent flow (3rd-party apps)**: apps navigate the browser to `/ui/apps/auth/?<standard OAuth authorize params>`. The page calls `GET /bodhi/v1/apps/access-requests/consent?<same query>` (session auth, Guest floor; `guest_endpoints` group) which validates only what BodhiApp depends on — app client via KC app-info, `redirect_uri` exact-match, duplicate-param rejection, and the app-facing scope vocabulary; `response_type`/`state`/PKCE are forwarded verbatim for Keycloak to enforce (`scope_user_*` role ceiling, `scope_apps:llms/mcps` section flags, `scope_access_request:*` reserved, all else passthrough) — returning an ok/error union whose `redirect_url` is set only when the redirect target was validated (RFC 6749 §4.1.2.1). `POST /bodhi/v1/apps/access-requests` is the single mutating call: deny (any session) returns an `error=access_denied&error_source=bodhi&state=…` redirect; approve (User+) re-validates, enforces role ceilings + grant-vs-scope + MCP ownership, creates the already-Approved row via `create_approved`, and returns the Keycloak authorize URL with the composed scope `openid profile email roles [passthrough…] scope_access_request:<resource-client-id>.<row-id>`. The page navigates to `redirect_url` unconditionally. Evaluation logic is shared between GET and POST (`apps/consent.rs::evaluate_consent_query`). The `token_service` scope-validation + `access_request_id` claim path is unchanged (full dotted scope string matched against the row).
