 ...
 */
test.describe('MCP Proxy — Everything Server via Inspector', { tag: ['@mcps', '@mcp-proxy', '@everything'] }, () => {
  ...
  test('MCP proxy full protocol journey — tools, resources, prompts via Inspector Direct mode', async ({
    ...
    await test.step('Open Inspector and configure Direct connection', async () => {
      ...
      await page.getByRole('textbox', { name: 'URL' }).fill(
        `${sharedServerUrl}/bodhi/v1/apps/mcps/${mcpId}/mcp`
      );
    ...
    await test.step('Tools — call get-tiny-image returns image', async () => {
      ...
      await expect(page.locator('img[src^="data:image"]').or(page.getByText('image/'))).toBeVisible();
    });
