...
- `#[napi(object)]` struct: `env_vars`, `app_settings`, `system_settings`, `client_id`, `client_secret`, `app_status`
- Free functions: `create_napi_app_options()`, `set_env_var()`, `set_app_setting()`, `set_system_setting()`, `set_client_credentials()`, `set_app_status()`
- `try_build_app_options_internal()` -- converts `NapiAppOptions` to `AppOptionsBuilder`, builds `Tenant` if client credentials provided
- Exports 20+ `BODHI_*` / `HF_*` / `DEFAULT_*` constants for JS usage
...
8. `update_with_option(&app_service, tenant)` -- persist tenant if provided
