...
- `models/router/` — model-router behavior: `RoutingStrategy` trait + `RouterContext` (`forward_one`, now also carries `time_service` + `health`) + `route_chat_completion()` (`strategy.rs`); status-only failure classification `classify_status() -> Disposition` (`classify.rs`, 2xx=Success, 400/422=Terminal, rest incl. 401/403/404/timeout=Retryable); `FallbackConfig::execute` (`fallback.rs`) does in-request fall-through **with cross-request health (Phase 3)**: orders enabled targets via `order_by_health` (not-cooled in declared order first, cooled last by soonest recovery — never-starve), tries them, returns the first Success/Terminal verbatim, falls through on Retryable / transport / structural-skip; a Success calls `health.record_success` (return-to-primary), a Retryable or genuine transport failure (`ModelRouterError::is_transport_failure`) calls `health.cooldown(key, cooldown_for(headers,cfg,now))`, while a **structural** skip (dangling/nested/unsupported) is NOT cooled; honors `max_attempts` (0=whole chain); on exhaustion returns the last upstream `Response` verbatim. Health is in-memory in `health.rs`: `HealthRegistry` trait (`is_cooled`/`cooled_until`/`cooldown`/`record_success`, `#[automock]`) + `DefaultHealthRegistry` (`RwLock<HashMap>`), keyed `target_key(tenant,alias,model)` so routers sharing a provider share its cooldown; `order_by_health` + `cooldown_for` (honors `Retry-After` when `honor_retry_after`) are free fns. Half-open recovery is passive (no probes): a cooldown is just a time window; the next real request that reaches an expired target is the trial. `HealthRegistry` is an `AppService` accessor (`health_registry()`), defaulted in the builder + `AppServiceStub`. `ModelRouterError` (`error.rs`); `ModelRouterService` (`service.rs`). Targets are resolved by alias identity (`alias_name()`: id for api, name for local) via `list_aliases`, NOT prefix-based `find_alias`.
...
- `app_access_requests/` — `AppAccessRequest` row + standard OAuth consent flow. `app_scopes.rs` owns the app-facing scope vocabulary (`parse_app_scope`, `compose_keycloak_scope`, `validate_grant_against_scope`, `match_redirect_uri`, `SCOPE_ACCESS_REQUEST_PREFIX`). Rows are created already-Approved at consent via `AccessRequestService::create_approved` (tenant/user bound at creation; `access_request_scope` stores the full dotted `scope_access_request:<resource-client-id>.<row-id>` the stateless Keycloak mapper parses). No Draft stage, no Keycloak-side registration; `latest_approved_for_app_user` backs the reauthorize prefill.
