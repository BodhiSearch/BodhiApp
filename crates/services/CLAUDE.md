...
- `models/router/` — model-router behavior: `RoutingStrategy` trait + `RouterContext` (`forward_one`) + `route_chat_completion()` (`strategy.rs`); status-only failure classification `classify_status() -> Disposition` (`classify.rs`, 2xx=Success, 400/422=Terminal, rest incl. 401/403/404/timeout=Retryable); `FallbackConfig::execute` does in-request fall-through (Phase 2): tries enabled targets in order, returns the first Success/Terminal verbatim, falls through on Retryable / transport / structural-skip (dangling alias, nested router, unsupported format), and on exhaustion returns the last upstream `Response` verbatim (or the typed error if no target produced one); honors `max_attempts` (0=whole chain); `cooldown_secs`/`honor_retry_after` stay persisted-but-unused (Phase 3) (`fallback.rs`); `ModelRouterError` (`error.rs`); `ModelRouterService` (`service.rs`). Targets are resolved by alias identity (`alias_name()`: id for api, name for local) via `list_aliases`, NOT prefix-based `find_alias`.
