use super::{
  ...
  BODHI_COMMIT_SHA, BODHI_DEPLOYMENT, BODHI_ENCRYPTION_KEY, BODHI_ENV_TYPE, BODHI_EXEC_LOOKUP_PATH,
  BODHI_EXEC_NAME, BODHI_EXEC_TARGET, BODHI_EXEC_VARIANT, BODHI_EXEC_VARIANTS, BODHI_HOME,
  BODHI_HOST, BODHI_KEEP_ALIVE_SECS, BODHI_LLAMACPP_ARGS, BODHI_LOGS, BODHI_LOG_LEVEL,
  ...
  HF_HOME, LOGIN_CALLBACK_PATH, PROD_DB, RUNPOD_POD_ID,
};
...
pub trait SettingService: std::fmt::Debug + Send + Sync {
  ...
  async fn session_db_url(&self) -> String {
    self
      .get_setting(BODHI_SESSION_DB_URL)
      .await
