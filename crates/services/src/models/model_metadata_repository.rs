...
use crate::models::{AliasSource, ModelMetadataEntity};
use sea_orm::prelude::*;
...
pub trait ModelMetadataRepository: Send + Sync {
  ...
  async fn list_model_metadata(&self, tenant_id: &str)
...
impl ModelMetadataRepository for DefaultDbService {
        ...
        Box::pin(async move {
          ...
          if metadata.api_model_id.is_none() {
            ...
            if let Some(ref filename) = metadata.filename {
              delete_query =
                delete_query.filter(model_metadata::Column::Filename.eq(filename.clone()));
            } else {
              delete_query = delete_query.filter(model_metadata::Column::Filename.is_null());
            }
            if let Some(ref snapshot) = metadata.snapshot {
              delete_query =
                delete_query.filter(model_metadata::Column::Snapshot.eq(snapshot.clone()));
            } else {
              delete_query = delete_query.filter(model_metadata::Column::Snapshot.is_null());
            }

            delete_query.exec(txn).await.map_err(DbError::from)?;
          }

          let model = model_metadata::ActiveModel {
            id: Set(id),
            tenant_id: Set(metadata.tenant_id.clone()),
          ...
          if metadata.api_model_id.is_some() {
            model_metadata::Entity::insert(model)
              .on_conflict(
                OnConflict::columns([
                  model_metadata::Column::TenantId,
      ...
      .with_tenant_txn(tenant_id, |txn| {
        Box::pin(async move {
          let result = model_metadata::Entity::find()
            .filter(model_metadata::Column::TenantId.eq(&tenant_id_owned))
            .filter(model_metadata::Column::Source.eq(AliasSource::Model))
            .filter(model_metadata::Column::Repo.eq(&repo_owned))
            .filter(model_metadata::Column::Filename.eq(&filename_owned))
            .filter(model_metadata::Column::Snapshot.eq(&snapshot_owned))
            .one(txn)
            .await
            .map_err(DbError::from)?;
  ...
  async fn batch_get_metadata_by_files(
    ...
    let tenant_id_owned = tenant_id.to_string();
    let files_owned: Vec<_> = files.to_vec();

    self
      .with_tenant_txn(tenant_id, |txn| {
        Box::pin(async move {
          let mut condition = Condition::any();
          for (repo, filename, snapshot) in &files_owned {
