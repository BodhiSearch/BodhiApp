use crate::db::{
  ...
  entities::app_instance,
  AppInstanceRepository, AppInstanceRow, DbError, DefaultDbService,
};
use objs::AppStatus;
...
impl AppInstanceRepository for DefaultDbService {
  async fn get_app_instance(&self) -> Result<Option<AppInstanceRow>, DbError> {
    let rows = app_instance::Entity::find()
      .all(&self.db)
      .await
  ...
  async fn upsert_app_instance(
    ...
    let active = app_instance::ActiveModel {
    ...
    app_instance::Entity::insert(active)
      .on_conflict(
        OnConflict::column(app_instance::Column::ClientId)
          .update_columns([
            app_instance::Column::EncryptedClientSecret,
            app_instance::Column::SaltClientSecret,
            app_instance::Column::NonceClientSecret,
            app_instance::Column::AppStatus,
            app_instance::Column::UpdatedAt,
          ])
          .to_owned(),
      )
      .exec(&self.db)
  ...
  async fn update_app_instance_status(
    ...
    let existing = app_instance::Entity::find_by_id(client_id.to_string())
      .one(&self.db)
      .await
      .map_err(DbError::from)?;
    ...
    app_instance::Entity::update(active)
      .exec(&self.db)
      .await
      .map_err(DbError::from)?;
  ...
  async fn delete_app_instance(&self, client_id: &str) -> Result<(), DbError> {
    app_instance::Entity::delete_by_id(client_id.to_string())
      .exec(&self.db)
      .await
      .map_err(DbError::from)?;
    Ok(())
