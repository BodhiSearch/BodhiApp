...
async fn test_create_and_get_mcp_server(
  ...
  let created = ctx.service.create_mcp_server(TEST_TENANT_ID, &row).await?;
  ...
  let fetched = ctx.service.get_mcp_server(TEST_TENANT_ID, "s1").await?;
...
async fn test_get_mcp_server_not_found(
  ...
  let result = ctx
    .service
    .get_mcp_server(TEST_TENANT_ID, "nonexistent")
    .await?;
...
async fn test_update_mcp_server(
  ...
  let updated = ctx
    .service
    .update_mcp_server(TEST_TENANT_ID, &updated_row)
    .await?;
  ...
  let fetched = ctx
    .service
    .get_mcp_server(TEST_TENANT_ID, "s1")
    .await?
    .unwrap();
...
async fn test_get_mcp_server_by_url(
  ...
  let found = ctx
    .service
    .get_mcp_server_by_url(TEST_TENANT_ID, "https://mcp.example.com/api")
    .await?;
  ...
  let not_found = ctx
    .service
    .get_mcp_server_by_url(TEST_TENANT_ID, "https://other.example.com")
    .await?;
...
async fn test_list_mcp_servers(
  ...
  let all = ctx.service.list_mcp_servers(TEST_TENANT_ID, None).await?;
  assert_eq!(2, all.len());

  let enabled = ctx
    .service
    .list_mcp_servers(TEST_TENANT_ID, Some(true))
    .await?;
  ...
  let disabled = ctx
    .service
    .list_mcp_servers(TEST_TENANT_ID, Some(false))
    .await?;
...
async fn test_count_mcps_by_server_id(
  ...
  let (enabled, disabled) = ctx
    .service
    .count_mcps_by_server_id(TEST_TENANT_ID, "s1")
    .await?;
  assert_eq!(2, enabled);
  ...
  let (e, d) = ctx
    .service
    .count_mcps_by_server_id(TEST_TENANT_ID, "nonexistent")
    .await?;
  assert_eq!(0, e);
  assert_eq!(0, d);
  Ok(())
...
async fn test_clear_mcp_tools_by_server_id(
  ...
  let affected = ctx
    .service
    .clear_mcp_tools_by_server_id(TEST_TENANT_ID, "s1")
    .await?;
  ...
  let m1 = ctx
    .service
    .get_mcp(TEST_TENANT_ID, "user-1", "m1")
    .await?
    .unwrap();
  ...
  let m2 = ctx
    .service
    .get_mcp(TEST_TENANT_ID, "user-1", "m2")
    .await?
    .unwrap();
  ...
  let m3 = ctx
    .service
    .get_mcp(TEST_TENANT_ID, "user-1", "m3")
    .await?
    .unwrap();
