...
mod tests {
  use crate::mcp::{
    validate_mcp_auth_config_name, validate_mcp_description, validate_mcp_server_description,
    validate_mcp_server_name, validate_mcp_server_url, validate_mcp_slug,
    validate_oauth_endpoint_url, RegistrationType, MAX_MCP_AUTH_CONFIG_NAME_LEN,
    MAX_MCP_DESCRIPTION_LEN, MAX_MCP_SERVER_NAME_LEN, MAX_MCP_SERVER_URL_LEN, MAX_MCP_SLUG_LEN,
  };
  use rstest::rstest;
  ...
  #[test]
  fn test_validate_mcp_auth_config_name_accepts_valid() {
    assert!(validate_mcp_auth_config_name("Header").is_ok());
    assert!(validate_mcp_auth_config_name("My OAuth Config").is_ok());
