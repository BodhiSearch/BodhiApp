...
| `services` | `crates/services/CLAUDE.md` | `crates/services/PACKAGE.md` | Business logic service traits. DB operations (SqliteDbService), TimeService (never Utc::now()), impl_error_from! macro. HubService, DataService, McpService, ToolService, AuthService, AccessRequestService (draft-first with requested_role/approved_role), AppInstanceService (no scope field). Error chain: service error→AppError→ApiError. Test infra: TestDbService, AppServiceStub builder, FrozenTimeService, OfflineHubService, SecretServiceStub |
