...
| `services` | `crates/services/CLAUDE.md` | `crates/services/PACKAGE.md` | Domain types + business logic (absorbed all former `objs` types). Domain types organized by module: auth/auth_objs.rs (ResourceRole, TokenScope, UserScope, AppRole, UserInfo), auth/auth_context.rs (AuthContext enum, AuthContextError - require_user_id() returns Result<&str, AuthContextError>), tokens/token_objs.rs (TokenStatus, ApiTokenRow), tokens/error.rs (TokenServiceError), models/model_objs.rs (Repo, HubFile, Alias, AliasSource, OAIRequestParams, GGUF, JsonVec, DownloadStatus, ApiKeyUpdate), settings/setting_objs.rs (Setting, EnvType, AppType, LogLevel), apps/app_objs.rs (AppStatus), users/user_objs.rs (UserAccessRequestStatus), toolsets/toolset_objs.rs, mcps/mcp_objs.rs (MAX_MCP_INSTANCE_NAME_LEN, validate_mcp_instance_name), app_access_requests/access_request_objs.rs (AppAccessRequestRow). shared_objs/ has SerdeJsonError, SerdeYamlError, ReqwestError, JsonRejectionError, ObjValidationError, token.rs. ApiError/OpenAIApiError/ErrorBody moved to routes_app::shared. Re-exports all errmeta types. Re-exports db::* via pub use db::* in lib.rs (use services::DbService not services::db::DbService). DB: DefaultDbService (SeaORM), TimeService (never Utc::now()). AppService::for_auth(ctx) -> AuthScopedAppService (in app_service/auth_scoped.rs). Auth-scoped services return domain errors: AuthScopedTokenService->TokenServiceError, AuthScopedMcpService->McpError, AuthScopedToolService->ToolsetError, AuthScopedUserService->AuthScopedUserError. Error chain: service error->AppError->ApiError (in routes_app). Test infra: TestDbService, SeaTestContext (dual SQLite/PG), AppServiceStub builder, FrozenTimeService, OfflineHubService, SecretServiceStub |
| `auth_middleware` | `crates/auth_middleware/CLAUDE.md` | `crates/auth_middleware/PACKAGE.md` | AuthContext enum defined in services crate, re-exported here (auth_context.rs is a re-export shim). Anonymous, Session, ApiToken, ExternalApp. require_user_id() returns AuthContextError (not ApiError). MiddlewareError (middleware_error.rs) is the return type for all 4 middleware functions - has blanket From<T: AppError> impl and IntoResponse. auth_middleware, optional_auth_middleware, api_auth_middleware, access_request_auth_middleware all return Result<Response, MiddlewareError>. ExternalApp.role from DB approved_role (not JWT scopes). AccessRequestValidator trait, AccessRequestAuthError enum (EntityNotApproved, etc.). JWT DefaultTokenService. CachedExchangeResult with role/access_request_id. Token digest. Same-origin CSRF. Test factories: test_session, test_external_app, test_external_app_no_role, RequestAuthContextExt |
...
| `routes_app` | `crates/routes_app/CLAUDE.md` | `crates/routes_app/PACKAGE.md` | API orchestration layer. ApiError, OpenAIApiError, ErrorBody defined here in shared/ (moved from services). Domain modules: tokens/, settings/, setup/, toolsets/, mcps/, models/, users/, apps/, auth/, api_models/, oai/, ollama/ (no routes_ prefix). Handler naming: Rails-style, no _handler suffix (tokens_index, tokens_create, tokens_destroy). ValidatedJson<T> extractor in shared/validated_json.rs. Domain-specific error enums per module (LoginError, AccessRequestError, McpValidationError, TokenRouteError, etc.) with errmeta_derive. OpenAPI utoipa registration checklist (7 steps). AuthContext from services crate (re-exported via auth_middleware). List response shapes: non-paginated use resource-plural field names ({mcps:[...]}, {toolsets:[...]}), paginated use {data:[], total, page, page_size}. Toolset/MCP CRUD, app access request workflow (draft-first, requested_role/approved_role) |
...
- **All API request handlers in `routes_app` use `AuthScopedAppService` as the primary extractor** via the `AuthScope` Axum extractor (defined in `crates/routes_app/src/shared/auth_scope_extractor.rs`). This replaces the older dual-extractor pattern of `Extension<AuthContext>` + `State(state)`.
- **Rule**: "All API request flows use `AuthScopedAppService`. Infrastructure (bootstrap, middleware, route composition) uses `AppService` directly."
- Exception: handlers that call `RouterState::forward_request()` (for LLM proxying) retain `State<Arc<dyn RouterState>>`.
...
- For error handling, follow the centralized error pattern: service errors → `ApiError` (in `routes_app::shared`) → OpenAI-compatible responses. Auth context errors use `AuthContextError` (in `services`). Middleware errors use `MiddlewareError` (in `auth_middleware`).
