          ...
          <FormItem>
            ...
            <FormControl>
              {Array.isArray(field) ? (
                <Input
                  {...formField}
                  value={formField.value ? (Array.isArray(formField.value) ? formField.value.join(',') : formField.value) : ''}
                  onChange={(e) => formField.onChange(e.target.value ? e.target.value.split(',').map(item => item.trim()) : undefined)}
